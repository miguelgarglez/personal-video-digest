...
Credential changes are private, user-only interactions. Ask the user to set the provider environment variable, or on macOS to run `video-digest config set api-key --provider opencode` (substituting their selected provider) or the corresponding `unset` in their own terminal. The agent must not invoke it, request the value, or observe the interaction.
