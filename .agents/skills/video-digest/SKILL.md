...
- Credential changes stay private. Tell the user to set the provider environment variable, or on macOS to run `video-digest config set api-key --provider opencode` (using their selected provider) or the corresponding `unset`; never invoke it or receive the secret.
